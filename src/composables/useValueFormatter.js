...
export function useValueFormatter(columnDefs, store) {
  const setValueFormatter = () => {
    ...
    columnDefs.value.forEach(
      (def) =>
        def.valueFormatter &&
        typeof def.valueFormatter === 'string' &&
        def.valueFormatter.match(regex) &&
        def.valueFormatter.substring(
          0,
          def.valueFormatter.indexOf(def.valueFormatter.match(regex)[0])
        ) === 'gui' &&
        (def.valueFormatter = getValueFormatter(def.valueFormatter))
    )
